...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The tidyverse Package that gives us a unified approach to importing, cleaning, transforming, visualizing, and modeling data.
...
str(waste_data$income_cat)  # Check structure
levels(waste_data$income_cat)  # Check factor levels
...
waste_data_long <- waste_data_fct %>% 
  pivot_longer(col = food_organic_waste:yard_garden_green_waste,
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    Well no I wouldnt really but this like this into a report, since he collum discriptions do not seem to be very human friendly to read. This in turn would change the collum names to make them easier understandable for humans before putting it into a report.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    Also here I would want to change the names so that it makes more sense overall, and be "easier understood"

3.  Render and fix any errors

```{r}

pivot_wider(waste_data_long_mean, names_from = waste_category, values_from = mean_percent)
```

# ßPart 3: Data visualization
